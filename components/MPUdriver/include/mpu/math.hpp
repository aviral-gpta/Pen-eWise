...
inline float_axes_t accelms2(const raw_axes_t& raw_axes, const accel_fs_t fs)
{
    float_axes_t axes;
    axes.x = raw_axes.x * accelResolution(fs)*9.81;
    axes.y = raw_axes.y * accelResolution(fs)*9.81;
    axes.z = raw_axes.z * accelResolution(fs)*9.81;
    return axes;
}
